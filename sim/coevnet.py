...
class coev:
	...
	def update_step(self, alpha = 0, weighted = False, verbose = False):
	...
	def mean_tension(self):
		return 1.0 * np.sum(self.tension_dict.values()) / self.m
	...
	def dynamics(self, nsteps = None, alpha = 0, weighted = False, sample = False, sample_interval = 100, verbose = False, verbose_interval = 1000, notify_end = True):
		...
		while not done:
			i += 1
			self.update_step(alpha, weighted = True, verbose = False)
			...
			if verbose: 
				if (i - 1) % verbose_interval == 0:
					print i, round(self.mean_tension(), 2)
...
def run_dynamics(N, c, k, alpha, normalize_k, rand_opinions,  **kwargs):
